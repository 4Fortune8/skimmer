...
4. Snapshots video statistics in batches of 50 IDs per request
   ...
   category, and the video's `defaultAudioLanguage`/`defaultLanguage` into
   `bronze_youtubeapi_video_stats`, limited to videos published within
   `--video-lookback-days` (default 30). The language fields come from the
   `snippet` part that is already requested, so they cost no extra quota.
...
`skimmer-video-languages` backfills those language fields onto videos that
were collected before the columns existed, sampling up to five untagged videos
per channel (~2,000 units for the whole corpus). Language is a property of the
video, so it is excluded from the deduplication digest and written to every
stored snapshot of the video; the backfill never overwrites a value the
...
normalization and language identification, and `services` orchestrates
long-running workers. `domain/language.py` is the single source of truth for
"what language is this?". It resolves a channel through a stack of
evidence and reports which tier decided in `language_source`: `audio`
(`defaultAudioLanguage`, what a viewer hears), `text` (`defaultLanguage`, only
the title's own script, and `en` on any transliterated title), `region` (a
non-western `en-IN` style tag), `script` (Devanagari/Arabic/CJK titles, which
outrank a typed `en`), `markers` (learned romanized vocabulary, the only tier
that reaches ASCII Bollywood titles that YouTube itself mislabels as English),
and `detected` (fastText lid.176, `make lid-model`). Any non-English tag
disqualifies a channel rather than being outvoted. The marker vocabulary is
generated by `scripts/train_language_markers.py` from channels YouTube labelled
with a South Asian audio language, so it is refreshed from data rather than
hand-maintained. Both the lead filter and the topics algorithm
read from this module rather than carrying their own word lists. Legacy data
